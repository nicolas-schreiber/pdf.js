...
async function getPageFromDocument(filename: string) {
  let doc = await pdfjs.getDocument(buildPathToPdf(filename));
...
describe('PDFDocumentProxy types', () => {
  ...
  test('has numPages', () => {
    expect(typeof doc.numPages).toBe('number');
  });
  ...
  test('get pdf info', () => {
    expect(typeof doc.pdfInfo.fingerprint).toBe('string');
    expect(typeof doc.pdfInfo.numPages).toBe('number');
  });
  ...
  test('get outline', async () => {
    ...
    expect(typeof node.bold).toBe('boolean');
    expect(typeof node.italic).toBe('boolean');
  ...
  test('get metadata', async () => {
    ...
    expect(typeof metadata.info['PDFFormatVersion']).toBe('string');
...
describe('PDFPageProxy', () => {
  ...
  test('pageInfo', () => {
    ...
    expect(typeof page.pageInfo.rotate).toBe('number');
    expect(typeof page.pageInfo.userUnit).toBe('number');
    expect(typeof page.pageInfo.ref).toBeDefined();
    expect(typeof page.pageInfo.ref.num).toBe('number');
    expect(typeof page.pageInfo.ref.gen).toBe('number');
    expectRect(page.pageInfo.view);
  });
  test('cleanupAfterRender', () => {
    expect(typeof page.cleanupAfterRender).toBe('boolean');
  });
  test('pendingCleanup', () => {
    expect(typeof page.pendingCleanup).toBe('boolean');
  });
  test('destroyed', () => {
    expect(typeof page.destroyed).toBe('boolean');
  });
  test('intentStates', () => {
    expect(typeof page.intentStates).toBe('object');
  });
  test('pageNumber', () => {
    expect(typeof page.pageNumber).toBe('number');
  });
  test('rotate', () => {
    expect(typeof page.rotate).toBe('number');
  });
  test('ref', () => {
    expect(page.ref).toBeDefined();
    expect(typeof page.ref.gen).toBe('number');
    expect(typeof page.ref.num).toBe('number');
  });
  test('userUnit', () => {
    expect(typeof page.userUnit).toBe('number');
  });
  test('view', () => {
  ...
  describe('getViewport', () => {
    ...
    test('PageViewport fields', () => {
      ...
      expect(typeof vp.rotation).toBe('number');
      ...
      expect(typeof vp.fontScale).toBe('number');
  ...
  test('get text content', async () => {
    ...
    expect(typeof item.dir).toBe('string');
    expect(typeof item.fontName).toBe('string');
    ...
    expect(typeof styles.ascent).toBe('number');
    expect(typeof styles.descent).toBe('number');
...
describe('PDFPageProxy annotation types', () => {
  ...
  function checkBaseAnnotation(annot: pdfjs.Annotation) {
    expect(typeof annot.annotationType).toBe('number');
    expect(typeof annot.annotationFlags).toBe('number');
    expect(typeof annot.hasAppearance).toBe('boolean');
    expect(typeof annot.id).toBe('string');
    expectTypedArray(annot.color!);
    expect(typeof annot.subtype).toBe('string');
    expectRect(annot.rect);
    expect(typeof annot.borderStyle.horizontalCornerRadius).toBe('number');
    expect(typeof annot.borderStyle.verticalCornerRadius).toBe('number');
    expect(typeof annot.borderStyle.width).toBe('number');
    expect(typeof annot.borderStyle.style).toBe('number');
  ...
  function checkWidgetAnnotation(widget: pdfjs.types.WidgetAnnotation) {
    ...
    expect(typeof widget.fieldType).toBe('string');
    expect(typeof widget.fieldFlags).toBe('number');
    expect(typeof widget.alternativeText).toBe('string');
    expect(typeof widget.defaultAppearance).toBe('string');
    expect(typeof widget.readOnly).toBe('boolean');
  }
  ...
  test('text widget', async () => {
    ...
    annotations.forEach(annotation => {
      ...
      expect(typeof widget.textAlignment).toBe('number');
      expect(typeof widget.multiLine).toBe('boolean');
      if (widget.maxLen !== null) {
        expect(typeof widget.maxLen).toBe('number');
      }
      expect(typeof widget.comb).toBe('boolean');
    });
    doc.destroy();
  });
...
describe('PDFPageProxy PDFObjects type', () => {
  function checkPDFObjects(pdfObjs: pdfjs.types.PDFObjects) {
    ...
    keys.forEach(key => {
      ...
      expect(typeof objs[key].resolved).toBe('boolean');
      expect(objs[key].data).toBeDefined();
    });
  }
...
describe('SVG types', () => {
  ...
  let savedVerbosity: pdfjs.PDFJS.VERBOSITY_LEVELS;

  asyncBeforeAll(async () => {
    ({ doc, page } = await getPageFromDocument('tracemonkey.pdf'));
    svg = new pdfjs.SVGGraphics(page.commonObjs, page.objs);
    // SVG output has some warning, setting verbodity to errors quiets them for
    // this test.
    savedVerbosity = pdfjs.PDFJS.verbosity;
  ...
  afterAll(() => {
    pdfjs.PDFJS.verbosity = savedVerbosity;
  });
});

describe('PDFJS global types', () => {
  test('has expected types for variables', () => {
    expect(typeof pdfjs.version).toBe('string');
    expect(typeof pdfjs.build).toBe('string');
    expect(typeof pdfjs.PDFJS.pdfBug).toBe('boolean');
    expect(typeof pdfjs.PDFJS.isLittleEndian).toBe('boolean');
  });
  ...
  test('PasswordException', () => {
    const ex = new pdfjs.PDFJS.PasswordException(
      'message',
      pdfjs.PasswordResponses.INCORRECT_PASSWORD
    );
    expect(ex).toEqual(jasmine.any(Error));
    expect(ex.message).toBe('message');
    expect(ex.name).toBe('PasswordException');
    expect(ex.code).toBe(pdfjs.PasswordResponses.INCORRECT_PASSWORD);
  });

  test('UnknownErrorException', () => {
    const ex = new pdfjs.PDFJS.UnknownErrorException('message', 'details');
    expect(ex).toEqual(jasmine.any(Error));
    expect(ex.message).toBe('message');
    expect(ex.name).toBe('UnknownErrorException');
  ...
  describe('Util functions', () => {
    test('Util', () => {
      expect(pdfjs.PDFJS.Util).toBeDefined();
    });

    test('makeCssRgb', () => {
      expect(pdfjs.PDFJS.Util.makeCssRgb(0, 0, 0)).toMatch(/^rgb\(.*\)$/);
    });
    ...
    test('transform', () => {
      expectTransform(pdfjs.PDFJS.Util.transform(m1, m2));
    });

    let pt: pdfjs.Point = [0.5, 0.7];
    test('applyTransform', () => {
      expectPoint(pdfjs.PDFJS.Util.applyTransform(pt, m1));
    });
    test('applyInverseTransform', () => {
      expectPoint(pdfjs.PDFJS.Util.applyInverseTransform(pt, m1));
    });

    let r: pdfjs.Rect = [0, 0, 1, 1];
    test('getAxialAlignedBoundingBox', () => {
      expectRect(pdfjs.PDFJS.Util.getAxialAlignedBoundingBox(r, m1));
    });

    test('inverseTransform', () => {
      expectTransform(pdfjs.PDFJS.Util.inverseTransform(m1));
    });

    test('singularValueDecompose2dScale', () => {
      expectArray(pdfjs.PDFJS.Util.singularValueDecompose2dScale(m1), 2);
    });
    ...
    test('apply3dTransform', () => {
      expectPoint3d(pdfjs.PDFJS.Util.apply3dTransform(m3d /*? $[0]*/, p3d));
    });

    test('normalizeRect', () => {
      expectRect(pdfjs.PDFJS.Util.normalizeRect(r));
    });
    ...
    describe('intersect', () => {
      test('should return a Rect if input rects intersect', () => {
        ...
        let intersection = pdfjs.PDFJS.Util.intersect(r1, r2);
      ...
      test('should be false if the input rects do not intersect', () => {
        ...
        expect(pdfjs.PDFJS.Util.intersect(r1, r2)).toBe(false);
      });
    });

    test('toRoman', () => {
      expect(typeof pdfjs.PDFJS.Util.toRoman(1024, false)).toBe('string');
    });

    test('appendToArray', () => {
      let arr = [0];
      pdfjs.PDFJS.Util.appendToArray(arr, [1]);
      expect(arr).toEqual([0, 1]);
    });

    test('prependToArray', () => {
      let arr = [0];
      pdfjs.PDFJS.Util.prependToArray(arr, [1]);
    ...
    test('extendObj', () => {
      let o1 = {};
      let o2 = { foo: 'bar' };
      pdfjs.PDFJS.Util.extendObj(o1, o2);
  ...
  test('maxImageSize', () => {
    expect(typeof pdfjs.PDFJS.maxImageSize).toBe('number');
  });
  ...
  test('VERBOSITY_LEVEL', () => {
    expect(pdfjs.PDFJS.VERBOSITY_LEVELS.errors).toBeDefined();
    expect(pdfjs.PDFJS.VERBOSITY_LEVELS.infos).toBeDefined();
    expect(pdfjs.PDFJS.VERBOSITY_LEVELS.warnings).toBeDefined();
  });
